 ...
 * @author
 *     Guy Bami W.
 * Created changes: 22.25.2017  
 * - added progressbar for uploading file
 * - added dialog modal to view file uploaded
 */
...
function generateEventPhotosDataGrid(targetDiv) {
    ...
    function fileNameFormatter(data, rowIndex) {
        var item = eventPhotosGrid.getItem(rowIndex);
        var formattedFileFullName = $.trim(item.fileFullName);
        if (formattedFileFullName == null || formattedFileFullName.length == 0)
            formattedFileFullName = "unknown";
        if (item) {
            var contains = false;
            for (i = 0; i < itemsArrayIds.length; i++) {
                if (itemsArrayIds[i].toString() == item.photoId.toString()) {
                    contains = true;
                    break;
                }
            }
            if (!contains){
                itemsArrayIds.push(item.photoId);
            }
            var html = '<label><a href="javascript:showPhotoModal(\'' + item.fileFullName + '\');" class="linkItemNameRow">'
                     +   'Voir Photo</a></label>';
            return html;
        }
        return data;
...
function addEventPhotos() {
    ...
    function uploadPhotosCompleted(data) {
        ...
        function assignPhotosToEventCompleted(data) {
            ...
            var itemToAdd = {};
            if (eventPhotosStore != null && jsonData != null) {
                $.each(jsonData, function(index, jsonItem){
                    if(jsonItem && jsonItem.insertedItemKey){
                        //alert('key: ' +  jsonItem.insertedItemKey );
                        itemToAdd.photoId = jsonItem.insertedItemKey;
                        itemToAdd.fileFullName = jsonItem.fileName;
                        itemToAdd.eventTitle = $("#eventId").find("option:selected").text();
...
function showPhotoModal(fileName){
    var img = '<img src="../../UploadedFiles/Images/Events/' + fileName + '"/>';
    //alert(img);
                            ...
                            + '<div class="modal-dialog modal-lg">'
                                + '<div class="modal-content">'
                                    + '<div class="modal-header">'
                                        ...
                                        + '<h4 class="modal-title" id="photoDialogTitle"> Photo</h4>'
                                    + '</div>'
