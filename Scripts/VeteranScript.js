                ...
                * Script to manage a  Veteran model entity.
                * @author
                *     Guy Bami W.
        ...
        // global varaibles
        var veteransStore = null;
        var veteransGrid = null;
        var currentItemIndex = -1;
        var itemsArrayIds = new Array();
        var currentVeteranId = -1;


        // some veteran actions commands
        var viewAllItemsCmd = "viewAllVeterans";
        ...
        var dataItemsGridId = "veteransGrid";
        var itemDetailsDivId = "veteranDetailsDiv";
        var itemsGridDivId = "veteransGridDiv";
        var itemDetailsFormId = "veteranDetailsForm";
        var veteranDetailsFormId = "veteranDetailsForm";
        ...
        var veteranDetailsDialogId = "veteranDetailsDialog";
        ...
        var controllerUrl = "../../Controllers/VeteranController.php";
        ...
        var subMenuItemSectionTitleLabel = "Veterans";
                    ...
                    try {
                        ...
                        var parsePageWidgets = function () {
                            ...
                            if (customUserRolesManagerObject != null && customUserRolesManagerObject.userRolesData.length > 0) {
                                ...
                                curmo.entityType = curmo.entityTypesObject.Veteran;
                                readItemRight = curmo.getAccessRightByEntityType(curmo.entityTypesObject.Veteran, curmo.accessRightsObject.readRight);
                            }
                            if (readItemRight == true) {
                                ...
                                generateVeteransDataGrid(itemsGridDivId);
                            }
                            else {
...
setFormFieldValue("name", "name-9c274b1");
setFormFieldValue("lastName", "lastName-bde7b64");
setFormFieldValue("email", "email-dcd2f3f");
setFormFieldValue("phoneNumber", "phoneNumber-742141c");
setFormFieldValue("zipCode", "zipCode-a99868f");
setFormFieldValue("city", "city-1d0d0d9");
setFormFieldValue("address", "address-1b976c5");
setFormFieldValue("position", "position-f531dc2");

}

       /**
        * generate dynamic Veterans datagrid
        * @param {string} targetDiv
        * @returns {none}
        */
        function generateVeteransDataGrid(targetDiv) {
 ...
 var positionColWidth = "auto";  
            ...
            if (customUserRolesManagerObject != null && customUserRolesManagerObject.userRolesData.length > 0) {
                var curmo = customUserRolesManagerObject; // short name
                curmo.entityType = curmo.entityTypesObject.Veteran;
                dataEditable = curmo.getAccessRightByEntityType(curmo.entityTypesObject.Veteran, curmo.accessRightsObject.editRight);
            }
            ...
            var xhrArgs = {
                ...
                error: function (errorMsg) {
                    ...
                    displayErrorContent("errorContentDiv", "Failed to get all veterans from server: " + errorMsg);
                    logError({ message: "Failed to get all veterans from server: \n" + errorMsg });
                }
            };
            var oldCellValue = "";
            ...
            function fetchDataCompleted(data) {
                ...
                veteransStore = new dojo.data.ItemFileWriteStore({
                    data: {
                        identifier: "veteranId",
                        items: jsonData
                    }
                });

                if (veteransStore != null) {
                    // save data store
                    veteransStore.save({
                    ...
                    veteransStore.comparatorMap = {};veteransStore.comparatorMap["gender"] = compareStringIgnoreCase; veteransStore.comparatorMap["name"] = compareStringIgnoreCase; veteransStore.comparatorMap["lastName"] = compareStringIgnoreCase; veteransStore.comparatorMap["email"] = compareStringIgnoreCase; veteransStore.comparatorMap["phoneNumber"] = compareStringIgnoreCase; veteransStore.comparatorMap["zipCode"] = compareStringIgnoreCase; veteransStore.comparatorMap["city"] = compareStringIgnoreCase; veteransStore.comparatorMap["address"] = compareStringIgnoreCase; veteransStore.comparatorMap["position"] = compareStringIgnoreCase;  
                    ...
                    // create datagrid
                    veteransGrid = new dojox.grid.EnhancedGrid({
                        id: dataItemsGridId,
                        store: veteransStore,
                    ...
                    // set sort index col
                    setSortColumnsIndexes(veteransGrid, 2, true);
                    // add events trigger
                    veteransGrid.on("SelectionChanged", reportSelection);
                    veteransGrid.on("StartEdit", gridStartEdit);
                    veteransGrid.on("ApplyCellEdit", gridApplyCellEdit);
                    veteransGrid.placeAt(targetDiv);
                    // parse datagrid
                    veteransGrid.startup();
                }
                ...
                function gridApplyCellEdit(inValue, inRowIndex, inFieldIndex) {
                    if (oldCellValue != null && inValue != oldCellValue) {
                        ...
                        obj["entityKeyId"] = storeItem["veteranId"].toString();
                        var itemId = storeItem["veteranId"].toString();
        ...
        function displayVeteranView(userAction) {

            var veteranId = currentVeteranId;
            debugMessageToConsole("veteranId:" + veteranId, highLevel);
            debugMessageToConsole("userAction: " + userAction, highLevel);

            var windowHeight = $(window).height();
            var windowWidth = $(window).width();

            var postParameters = { "veteranId": veteranId, "userAction": userAction };
            var xhrArgs = {
                url: controllerUrl,
                postData: postParameters,
                ...
                method: postMethod,
                error: function (errorMsg) {
                    logError({ message: "Reponse failed to get veteran details with error: " + errorMsg });
                }
            };
            ...
            function fetchDataCompleted(data) {
                ...
                if (jsonData == null) {
                    displayErrorContent(errorContentDivId, "displayVeteranView: " + jsonErrorMsg);
                    return;
                }
                else if (jsonData.length == 1 && jsonData[0].jsonErrorMessage != null) {
                    displayErrorContent(errorContentDivId, jsonData[0].jsonErrorMessage);
        ...
        function viewVeteranDetails(veteranId) {
            currentVeteranId = veteranId;
            currentItemIndex = getCurrentItemIndex(veteranId);
            displayVeteranView(viewItemDetailsCmd);
        }
        ...
        function createToolbarBtns(userAction) {
            ...
            if (customUserRolesManagerObject != null && customUserRolesManagerObject.userRolesData.length > 0) {
                ...
                if (!customUserRolesManagerObject.rolesLoaded) {
                    // load roles only once
                    curmo.entityType = curmo.entityTypesObject.Veteran;
                    addNewItemRight = curmo.getAccessRightByEntityType(curmo.entityTypesObject.Veteran, curmo.accessRightsObject.createRight);
                    editItemRight = curmo.getAccessRightByEntityType(curmo.entityTypesObject.Veteran, curmo.accessRightsObject.editRight);
                    deleteItemRight = curmo.getAccessRightByEntityType(curmo.entityTypesObject.Veteran, curmo.accessRightsObject.deleteRight);
        ...
        */
        function deleteItemsBtnClick() {
            ...
            if (veteransGrid != null) {
                var items = veteransGrid.selection.getSelected();
                if (items.length > 0) {
        ...
        */
            ...
            if (veteransGrid != null) {
                ...
                if (items.length > 0) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        itemId = veteransGrid.store.getValue(items[i], "veteranId");
                        itemId = $.trim(itemId);
                    ...
                    // now delete items
                    deleteSeletedVeterans(selectedIdsArray);
                }
            }
        ...
        function registerButtonClickEvents() {
            ...
            $("#" + addNewItemBtnDialogId).click(function () {
                validateBeforeAddNewVeteran();
            });
            $("#" + editItemBtnDialogId).click(function () {
                displayVeteranView(editItemDetailsCmd);
            });
            ...
            $("#" + cancelChangesBtnDialogId).click(function () {
                displayVeteranView(cancelChangesCmd);
            });
        ...
        function validateBeforeAddNewVeteran() {

            var isFormInputsValid = $("#" + veteranDetailsFormId).valid();
        ...
        function showAddItemDialog() {
            ...
            if ($("#" + veteranDetailsDialogId) != null) {
                // set content dialog
                ...
                $("#" + veteranDetailsDialogId).modal("show");
            }
        }
        ...
        function closeAddEditItemDialog() {
            if ($("#" + veteranDetailsDialogId) != null) {
                $("#" + veteranDetailsDialogId).modal("hide");
            }
        }
        ...
        function addNewVeteran() {

            var veteranDetailsFormObject = $("#" + veteranDetailsFormId).serializeObject();
            // convert form to json object
            var itemToAdd = veteranDetailsFormObject;
            itemToAdd = veteranDetailsFormObject;
            var postParameters = {
                "formValues[]": dojo.toJson(veteranDetailsFormObject, true)
            ...
            //get postback result
            function addNewVeteranCompleted(data) {

                debugMessageToConsole("addNewVeteranCompleted data : " + data, highLevel);
                // get json result
                var jsonData = data;
                // clear error message
                $("#" + errorContentDivId).html("");
                if (jsonData == null) {
                    displayErrorContent(errorContentDivId, "addNewVeteranCompleted: " + jsonErrorMsg);
                    return;
                }
                else if (jsonData.length == 1 && jsonData[0].jsonErrorMessage != null) {
                    displayErrorContent(errorContentDivId, jsonData[0].jsonErrorMessage);
                ...
                if (veteransStore != null && jsonData[0].insertedItemKey != null) {
                    itemToAdd.veteranId = jsonData[0].insertedItemKey;
                    // redisplay data grid    
                    veteransStore.newItem(itemToAdd);
                }
            }

            // send async xhr request to server
            sendAsyncRequest(xhrArgs.url, xhrArgs.postData, xhrArgs.handleAs, xhrArgs.method,
                xhrArgs.error, addNewVeteranCompleted);

        }


        function submitInlineGridChanges(veteranId, jsonValues) {
        ...
        function deleteSeletedVeterans(veteranIdsList) {
            var postParameters = {
                "selectedIds[]": veteranIdsList,
            ...
            // send async xhr request to server
            sendAsyncRequest(xhrArgs.url, xhrArgs.postData, xhrArgs.handleAs, xhrArgs.method,
                xhrArgs.error, deleteSeletedVeteransCompleted);
            //get postback result
            function deleteSeletedVeteransCompleted(data) {

                var jsonData = data;
                $("#" + errorContentDivId).html("");
                if (jsonData == null) {
                    displayErrorContent("errorContentDiv", "deleteSeletedVeteransCompleted: " + jsonErrorMsg);
                    return;
                }
                else if (jsonData.length == 1 && jsonData[0].jsonErrorMessage != null) {
                    displayErrorContent(errorContentDivId, jsonData[0].jsonErrorMessage);
                ...
                var itemsToDelete = veteransGrid.selection.getSelected();
                $.each(itemsToDelete, function (index, item) {
                    if (item) {
                        veteransStore.deleteItem(item);
                    }
                });
                // save datastore
                veteransStore.save();
        ...
        function submitVeteranDetailsForm(veteranId) {
            //check form validation
            //if (!veteranDetailsForm.validate()) {
            //    return;
            //}
            // convert form to json object
            var veteranDetailsFormObject = $("#" + veteranDetailsFormId).serializeObject();
            ...
            var jsonValues = dojo.toJson(veteranDetailsFormObject);
            ...
            // send async xhr request to server
            sendAsyncRequest(xhrArgs.url, xhrArgs.postData, xhrArgs.handleAs, xhrArgs.method,
                xhrArgs.error, submitVeteranDetailsCompleted);
            //get postback result
            function submitVeteranDetailsCompleted(data) {
                //debugMessageToConsole(" VeteranDetailsForm-postback: " + $.trim(data), highLevel);
                var jsonData = data;
                $("#" + errorContentDivId).html("");
                if (jsonData == null) {
                    displayErrorContent(errorContentDivId, "submitVeteranDetailsCompleted: " + jsonErrorMsg);
                    return;
                }
                else if (jsonData.length == 1 && jsonData[0].jsonErrorMessage != null) {
                    displayErrorContent(errorContentDivId, jsonData[0].jsonErrorMessage);
                ...
                // get item to update
                var itemToUpdate = veteransGrid.getItem(currentItemIndex); 

                

veteransStore.setValue(itemToUpdate, "gender", veteranDetailsFormObject.gender);
veteransStore.setValue(itemToUpdate, "name", veteranDetailsFormObject.name);
veteransStore.setValue(itemToUpdate, "lastName", veteranDetailsFormObject.lastName);
veteransStore.setValue(itemToUpdate, "email", veteranDetailsFormObject.email);
veteransStore.setValue(itemToUpdate, "phoneNumber", veteranDetailsFormObject.phoneNumber);
veteransStore.setValue(itemToUpdate, "zipCode", veteranDetailsFormObject.zipCode);
veteransStore.setValue(itemToUpdate, "city", veteranDetailsFormObject.city);
veteransStore.setValue(itemToUpdate, "address", veteranDetailsFormObject.address);
veteransStore.setValue(itemToUpdate, "position", veteranDetailsFormObject.position);
        ...
        function viewNextItemDetails() {
            ...
            var veteranId = itemsArrayIds[currentItemIndex];
            viewVeteranDetails(veteranId);
        }
        ...
        function saveVeteranChanges() {
            submitVeteranDetailsForm(currentVeteranId);
            if ($("#" + viewItemDetailsDialogId) != null) {
                $("#" + viewItemDetailsDialogId).modal("hide");
            }
        }
