...
class Utils {
    ...
    public static function generateTableArray($array) {
        ...
        foreach ($array as $key => $value) {
            if (is_array($value)) {
                $buf .= '<tr>' . Utils::generateTableArray($value) . '</tr>';
            } else
                $buf .= "<td>$value</td>";
        }
        return $buf;
    ...
    public static function sendMailWithSender($senderName, $subject, $message) {
        ...
        // send email
        $canSend = false;
        $rootURL = self::getWebsiteRootURL();
        if (strpos($rootURL, "localhost") !== false)
            $canSend = false;
        else
            $canSend = true;
    ...
    public static function checkAndStartSession() {
        ...
        //echo "Calling checkAndStartSession with values....<br />";
        ...
        // set default session data for Test
